...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        fprintf(stderr,"\nError: Data directory %s needs to exist before calling aksyonchaind-cold. Exiting...\n\n",mapArgs["-datadir"].c_str());
        return false;        
    }
        
    if(!mc_DoesParentLogDirExist())
    {
        fprintf(stderr,"\nError: Log directory %s needs to exist before calling aksyonchaind-cold. Exiting...\n\n",mapArgs["-logdir"].c_str());
        return false;        
    }
        ...
        (mc_gState->m_Params->NetworkName() == NULL))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  aksyonchaind-cold <blockchain-name> [options]                     " + _("Start AksyonChain Offline Daemon") + "\n";
    ...
    if(err)
    {
        ...
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Please restart aksyonchaind with reindex=1.\n",mc_gState->m_Params->NetworkName());
        }
        else
        {
            fprintf(stderr,"\nERROR: Couldn't initialize permission database for blockchain %s. Probably aksyonchaind for this blockchain is already running. Exiting...\n",mc_gState->m_Params->NetworkName());
        }
        delete mc_gState;                
        return false;
    }
    ...
    if(mc_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL)
    {
        ...
        if(mc_gState->IsSupported(protocol_version) == 0) 
        {
            ...
            {
                fprintf(stderr,"ERROR: The protocol version (%d) for blockchain %s has been deprecated and was last supported in AksyonChain %s\n\n",
            ...
            {
                fprintf(stderr,"ERROR: Parameter set for blockchain %s was generated by AksyonChain running newer protocol version (%d)\n\n",
                        mc_gState->m_Params->NetworkName(),protocol_version);                        
                fprintf(stderr,"Please upgrade AksyonChain\n\n");
                delete mc_gState;                
                return false;
            }
        }
